...
We add 1 for the log transformed variables in case there is a row with 0, as log(0) is undefined. Adding one makes it log(1) which is equal to 0.

**Question:** In complete sentences, what is the purpose of adding 1 in the creation of the duration variable?

We add 1 when creating the duration variable in case there is a row that has year 2018. It would not make sense for duration to be 0, and we would rather it just be 1.
...
```{r,eval=T}
ggplot(bgg3) +
  geom_point(aes(geek_rating, num_votes))

ggplot(bgg3) +
  geom_point(aes(geek_rating, owned))

ggplot(bgg3) +
  geom_point(aes(geek_rating, log_vote))

ggplot(bgg3) +
  geom_point(aes(geek_rating, log_own))
```

**Question:** In complete sentences, describe how the relationship changes when you take the log of the independent variable.

The linearity of the relationship between geek_rating and the log of the other two improves. Before, it appeared very flat, but afterwards looks like a pretty strong, positive relationship.
...
bgg4 = bgg3 %>%
        mutate(Set=sample(x = c("Train", "Test"), size = 4999, replace = TRUE, prob = c(0.8, 0.2)))
...
```{r,eval=T}
MAE.func(test.bgg2$lm1res)
MAE.func(test.bgg2$lm2res)
MAE.func(test.bgg2$lm3res)
```

**Question:** Which model does the best job at predicting the geek rating of these board games?

It appears that `lm3` does the best job at predicting the geek rating of these board games.
