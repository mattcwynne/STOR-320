...
Write code that replaces the observations in the vector `x` that are greater than or equal to `0` with a string of characters `"non-negative"` and the observations that are smaller than `0` with a string of characters `"negative"`. Hint: try `ifelse()` function. Print out the first five values in this new version of `x`.
...
Write `for`-Loop to count how many observations in the vector `x` are non-negative and how many observations are negative. (There are many easier ways to solve this problem. Use `for`-Loop or get 0 points. Use the `cat()` function to print out a sentence that states how many non-negative and negative obesrvations there are. For example, "The number of non-negative observations is 32".

```{r}
non_negative = 0
negative = 0
for(i in x){
  if(i == "non-negative") non_negative = non_negative + 1
  if(i == "negative") negative = negative + 1
}
cat("the number of non-negative observations is", non_negative, "\n")
cat("the number of non-negative observations is", negative)
```
...
```{r,eval=T}
A = matrix(1:1000000, 100000, 10, byrow = TRUE) # DO NOT CHANGE
sum_row = vector()
for(i in 1:100000){
  sum_row[i] = sum(A[i,])
}
...
for(i in 1:ncol(iris)){
  cat("The number of unique values for", names(iris)[i], "is", nrow(unique(iris[i])), "\n")
}
```
