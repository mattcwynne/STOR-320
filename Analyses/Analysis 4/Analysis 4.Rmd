...
BEST5.DATA2 = BEST5.DATA %>%
  gather(First, Second, Third, Fourth, Fifth, key = Model, value = Predict, factor_key = TRUE)
...
ggplot(BEST5.DATA2) +
  ...
  geom_line(aes(D, Predict, color = Model)) +
  theme_minimal() +
  xlab("Julian Day") +
  ylab("Max Water Temperature")
```

Answer *(2 Points)*: Looking at the graph above, it appears that most of the models are on top of one another, indicating that they are predicting similar values.
...
Answer: Since we want the lowest avgMAE, I would recommend `I = 4` and `J = 7`, as these show the lowest avgMAE on the graphs above.
...
BEST.func=function(data){
  OUT = data[which.min(data$MAE), 1:2]
  as.numeric(OUT)
}
...
```{r,eval=T}
ggplot(DATA) +
  geom_point(aes(W, predict(train.model.func(DATA, 2, 6))), alpha = 0.05, stroke = 0) +
...
```{r,eval=T}
ggplot(DATA) +
  geom_point(aes(D, residuals(train.model.func(DATA, 2, 6))), alpha = 0.05, stroke = 0) +
